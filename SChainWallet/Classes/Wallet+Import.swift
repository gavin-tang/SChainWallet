...
public extension Wallet {
    ...
    static func privateKeyWithAddress(address: String, password: String) -> String? {
        var wallet: Wallet?
        ...
        do {
            return try wallet?.exportWallet(password: password)
        } catch {}
        return nil
    }

    static func removeWalletWithAddress(address: String) -> Bool {
        let isExist = Wallet.isAlreadyExist(address)
        if !isExist {
            return false
        }
